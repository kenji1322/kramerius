...
public class VirtualCollectionViewObject {
	...
	public List<CollectionItemViewObject> getVirtualCollections() throws Exception {
		return CollectionItemViewObject.wrap(this.solrManager.getCollections(), this.fedoraAccess);
	}
	...
	public List<CollectionItemViewObject> getVirtualCollectionsFromFedora() throws Exception {
		...
		return CollectionItemViewObject.wrap(collections, this.fedoraAccess);
	}

	public List<CollectionItemViewObject> getVirtualCollectionsLocale() throws Exception {
		SortType selectedVal = sortType();
		if (selectedVal != null) {
			return CollectionItemViewObject.onlyLocalizedDescriptions(this.localeProvider.get(),
					this.solrManager.getSortedCollections(this.localeProvider.get(), selectedVal), this.fedoraAccess);
		} else {
			return CollectionItemViewObject.onlyLocalizedDescriptions(this.localeProvider.get(),
					this.solrManager.getCollections(), this.fedoraAccess);
		}
	}
	...
	public List<CollectionItemViewObject> getVirtualCollectionsFromFedoraLocale() throws Exception {
		...
		if (selectedVal != null) {
			return CollectionItemViewObject.onlyLocalizedDescriptions(this.localeProvider.get(),
					this.fedoraManager.getSortedCollections(this.localeProvider.get(), selectedVal), this.fedoraAccess);
		} else {
			return CollectionItemViewObject.onlyLocalizedDescriptions(this.localeProvider.get(),
					this.fedoraManager.getCollections(), this.fedoraAccess);
		}
	}
	...
	public List<String> getHomeTabs() throws Exception {
		...
		// Mozne hodnoty
		// custom,mostDesirables,newest,facets,browseAuthor,browseTitle,info
		// Pokud mame nastavenou sbirku NEzobrazime mostDesirables, custom,
		// browseAuthor,browseTitle
		ArrayList<String> validTabs = new ArrayList<String>();
		Collection vc = getCurrent();
		for (String tab : tabs) {
			if (vc == null || tab.equals("info") || tab.equals("facets") || tab.equals("newest")
	...
	private String[] filterLogged(String[] tabs) {
		List<String> mustBeLoggedList = new ArrayList<String>(
				Arrays.asList(kConfiguration.getPropertyList("search.home.tabs.onlylogged")));
