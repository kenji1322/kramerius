...
public class VirtualCollectionServlet extends AbstractColsServlet {

	public static final java.util.logging.Logger LOGGER = java.util.logging.Logger
			.getLogger(VirtualCollectionServlet.class.getName());
	...
	enum PostActions {
		...
		LONG_TEXT_UPLOAD {
			...
			@Override
			void doPerform(AbstractColsServlet vc, FedoraAccess fedoraAccess, CollectionsManager colMan,
				...
				CollectionUtils.modifyLangDatastream(collection, language,
						AbstractCollectionManager.LONG_TEXT_DS_PREFIX + language, new String(decoded, "UTF-8"),
						fedoraAccess);
			...
			void doPerform(AbstractColsServlet vc, FedoraAccess fedoraAccess, CollectionsManager colMan,
				...
				while (iterator.hasNext()) {
					...
					if (!fileItemStream.isFormField()) {
						...
						CollectionUtils.modifyImageDatastream(collection, streamName, contentType, bos.toByteArray(),
								fedoraAccess);
					} else {
		...
		abstract void doPerform(AbstractColsServlet vc, FedoraAccess fedoraAccess, CollectionsManager colMan,
				HttpServletRequest req, HttpServletResponse response) throws Exception, SecurityException;
	}
	...
	enum Actions {
		...
		CHECK {
			...
			@Override
			void doPerform(AbstractColsServlet vc, FedoraAccess fedoraAccess, CollectionsManager colMan,
					HttpServletRequest req, HttpServletResponse resp) throws IOException, SecurityException {
		...
		ADDTOCOLLECTIONS {
			...
			void doPerform(AbstractColsServlet vc, FedoraAccess fedoraAccess, CollectionsManager colMan,
				...
				String url = base + "?action=start&def=reindex&out=text&params=fromKrameriusModel," + pid + "," + pid
		...
		/**
		 * Request to write to response the content of parameter content. Used
		...
		CREATE {
			...
			void doPerform(AbstractColsServlet vc, FedoraAccess fedoraAccess, CollectionsManager colMan,
				...
				String pid = CollectionUtils.create(fedoraAccess, null, canLeave, plainTexts,
						new CollectionUtils.CollectionManagerWait(colMan));
				resp.setContentType("text/plain");
		...
		CHANGE {
			...
			void doPerform(AbstractColsServlet vc, FedoraAccess fedoraAccess, CollectionsManager colMan,
				...
				// String k4url = url.getProtocol() + "://" + url.getHost() +
				// ApplicationURL.extractPort(url) + req.getRequestURI();
